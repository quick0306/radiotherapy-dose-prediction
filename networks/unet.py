...
class DownBlock(nn.Module):
    ...
    def __init__(self,
        ...
        self.conv2 = get_conv_layer(self.out_channels+self.in_channels, self.out_channels, kernel_size=3, stride=1, padding=self.padding,
                                    bias=True, dim=self.dim)
...
class UpBlock(nn.Module):
    ...
    def forward(self, encoder_layer, decoder_layer):
        ...
        if self.normalization:
            y = self.norm1(y)  # normalization 1
        z = self.concat(merged_layer, y)
        y = self.conv2(z)  # convolution 2
...
class UNet(nn.Module):
    ...
    def __init__(self,
        ...
        # create encoder path
        num_filter_last = self.in_channels
        for i in range(self.n_blocks):
            num_filters_in = num_filter_last 
            num_filters_out = self.out_channels
            num_filter_last = num_filters_out + num_filters_out 
